...
def init_db(conn: sqlite3.Connection) -> None:
	...
	# best30: добавили best_snapshot_id — снапшот, в который был зафиксирован лучший прирост (diff)
	conn.execute("""
	...
	""")
	# миграция, если база старая и колонки ещё нет
	ensure_column(conn, "best30", "best_snapshot_id", "TEXT")
...
def compute_best30(conn: sqlite3.Connection, best_for_snapshot_id: str,
	...
	"""
	Считает лучший прирост (diff) за окно и сохраняет:
	  - diff
	  - best_snapshot_id: снапшот (sid), в который случился этот лучший прирост (prev -> sid)
	"""
	...
	# param -> pid -> (diff, name, level, pid, best_sid)
	best: Dict[str, Dict[int, Tuple[int, str, int, int, str]]] = {p: {} for p in BEST_PARAMS}
			...
			for pid in common:
				...
				for param, diff in diffs.items():
					...
					if old is None or diff > old[0]:
						# best_snapshot_id = sid (в который пришли после шага prev -> sid)
						best[param][pid] = (diff, name, level, pid, sid)
	...
	for param, by_pid in best.items():
		...
		conn.executemany(
			"INSERT OR REPLACE INTO best30(best_for_snapshot_id, param, pid, name, level, diff, best_snapshot_id) "
			"VALUES(?,?,?,?,?,?,?)",
			[(best_for_snapshot_id, param, pid, name, level, diff, best_sid)
			 for (diff, name, level, pid, best_sid) in rows]
		)
